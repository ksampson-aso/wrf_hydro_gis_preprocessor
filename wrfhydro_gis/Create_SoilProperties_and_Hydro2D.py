...
add_vars = ['ssi', 'snowretfac', 'tau0', 'rsurfsnow', 'scamax']
...
def ApplyPedo(sand, clay, orgm=0.0):
    # Calcs
    theta_1500t = (-0.024*sand) + (0.487*clay) + (0.006*orgm) + (0.005*(sand*orgm)) - (0.013*(clay*orgm)) + (0.068*sand*clay) + 0.031
    theta_1500  = theta_1500t + ((0.14*theta_1500t) - 0.02)
    theta_33t   = (-0.251*sand) + (0.195*clay) + (0.011*orgm) + (0.006*(sand*orgm)) - (0.027*(clay*orgm)) + (0.452*sand*clay) + 0.299
    theta_33    = theta_33t + ((1.283*theta_33t*theta_33t) - (0.374*theta_33t) - 0.015)
    theta_s33t  = (0.278*sand) + (0.034*clay) + (0.022*orgm) - (0.018*(sand*orgm)) - (0.027*(clay*orgm)) - (0.584*sand*clay) + 0.078
    theta_s33   = theta_s33t + ((0.636*theta_s33t) - 0.107)
    psi_et      = (-21.67*sand) - (27.93*clay) - (81.97*theta_s33) + (71.12*(sand*theta_s33)) + (8.29*(clay*theta_s33)) + (14.05*sand*clay) + 27.16
...
def obtain_soilparams(in_file, table_name='STAS'):
    ...
    for n in range(num_tables):
        ...
        if in_table_name == table_name:
            ...
            header += [item for item in soilparm_columns if item not in header]    # Add columns to the end that may be missing
...
def obtain_HYDROparams(in_file):
    ...
    sfcRough_df = pd.read_table(StringIO('\n'.join(SFC_ROUGH_vals)), sep=',', header=None, names=('OV_ROUGH2D','descrip'))
    ...
    for param in paramList:
        ...
        if paramName in soilTblDf.columns.values:
            ...
            if useSoilComp and 'SOILCOMP' in rootgrp_geo.variables.keys():
                ...
                maskArr = array_replace(replaceArr[0,:], np.full(soilTblDf.solID.values.shape, False, dtype=bool), maskArr)
    ...
    if updateTexture:
        ...
        for layer,layerOpts in lyrDict.items():
            ...
            if useSoilComp and 'SOILCOMP' in rootgrp_geo.variables.keys():
                ...
                soil_texture[((clay[i] >= 7) & (clay[i] < 20)) & (sand[i] > 52) & (((silt[i] + 2*clay[i]) >= 30) | ((clay[i] < 7) & (silt[i] < 50) & ((silt[i]+2*clay[i]) >= 30)))] = 3
                soil_texture[((clay[i] >= 7) & (clay[i] < 27)) & ((silt[i] >= 28) & (silt[i] < 50)) & (sand[i] <= 52)] = 6
                soil_texture[((silt[i] >= 50) & ((clay[i] >= 12) & (clay[i] < 27))) | (((silt[i] >= 50) & (silt[i] < 80)) & (clay[i] < 12))] =  4
            ...
            if setUrban:
                for param_to_alter, replaceval in zip(['SMCMAX1', 'SMCREF1', 'SMCWLT1'], [0.45, 0.42, 0.40]):
                    if param == param_to_alter:
                        ...
                        print(f'        Replaced HYDRO2D parameter {param} with {replaceval} where vegetation is urban but soil is not soilwater')
...
if __name__ == '__main__':
    ...
    geoFile = args.in_Geogrid = os.path.abspath(args.in_Geogrid)                # Obtain absolute path for required input geogrid file..
